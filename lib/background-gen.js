...
function buildBackgroundPrompt(sceneDescription, brandStyle, referenceStyleSummary) {
  ...
  const refStyle = referenceStyleSummary
    ...
    : '';
  return `${sceneDescription}${style}${refStyle}\n\n${HARD_CONSTRAINTS}`;
}
...
async function generateBackground(row, sceneHint, brandStyle) {
  ...
  // 查 reference 资产，静默失败不影响主流程
  let referenceStyleSummary = null;
  try {
    const refs = await listReferenceAssets();
    if (refs.length > 0) {
      referenceStyleSummary = await analyseReferenceStyle(refs);
    }
  } catch (_) {}

  const prompt = buildBackgroundPrompt(description, brandStyle, referenceStyleSummary);
